...
//   let [createdBy, setCreatedBy] = useState("");
  ...
  let handleClick = async (e) => {
    e.preventDefault();
    try {
      let res = await axios.put(`${url}/manage-timing/${id}`, {
        Name,
        timing,
        status,
        // mobile,
  ...
  let getData = async () => {
    try {
      let res = await axios.get(`${url}/manage-timing/${id}`);

      if (res.status === 200) {
        setName(res.data.timing.Name);
        setTiming(res.data.timing.timing);
        setStatus(res.data.timing.status);
